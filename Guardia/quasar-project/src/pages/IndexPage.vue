      ...
      <q-item v-for="(actividad, index) in actividadesFiltradas" :key="index" :class="{'bg-grey-4': index % 2 === 0}">
        ...
        <q-item-section class="col-1 text-center">
          <div style="display: flex; align-items: center; justify-content: center;">
            ...
            <q-btn v-if="!actividad.entradaConfirmada" flat round icon="check" color="green" @click="confirmarHoraEntrada(index)" />
            <q-btn v-if="!actividad.entradaConfirmada" flat round icon="close" color="red" @click="marcarHoraActualEntrada(index)" />
          </div>
        </q-item-section>
        <q-item-section class="col-1 text-center">
          <div style="display: flex; align-items: center; justify-content: center;">
            {{ actividad.horaSalida }}
            <q-btn v-if="!actividad.salidaConfirmada" flat round icon="check" color="green" @click="confirmarHoraSalida(index)" />
            <q-btn v-if="!actividad.salidaConfirmada" flat round icon="close" color="red" @click="marcarHoraActualSalida(index)" />
          </div>
        </q-item-section>
        <q-item-section class="col-2 text-center">
